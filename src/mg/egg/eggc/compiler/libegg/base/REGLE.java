...
public class REGLE implements Serializable {
	...
	/**
	 * la liste de symbôles en partie droite de la règle de producion.
	 */
	...
	/**
	 * Renvoie une énumération des symbôles en partie droite de la règle de
	 * production.
	 */
	...
	/**
	 * Calcule les premiers d'une chaîne de symbôles à l'ordre k.
	 * 
	 * @param n
	...
	public boolean add_action(String n) {
		// devrait etre apres la recherche !
		int p = ++lastpos;
	...
	public void init() {
		/*
		 * pour chaque SymbREGLE s de la règle insérer les attributs de s dans
		 * la table de la règle initialiser les hérités de gauche à "initialisé"
		 * initialiser les synthétisés de gauche à "non initialisé" initialiser
		 * les hérités de droite à "non initialisé" initialiser les synthétisés
		 * de droite à "initialisé"
		 */
		init_symb(gauche);
	...
	public void autos(IAction l) {
		/*
		 * pour chaque ActRegle qui est auto engendrer le code de mise à jour
		 * des attributs
		 */
		for (Enumeration<EltREGLE> e = elements(); e.hasMoreElements();) {
			EltREGLE er = (EltREGLE) e.nextElement();
		...
		for (Enumeration<ATTRIBUT> eg = g_attributs.elements(); eg
			...
			for (Enumeration<EltREGLE> e = droite.elements(); e
				...
				if (er instanceof SymbREGLE) {
					...
					if (sa != null) { // oui
						if (sa.getEtat()) {
							sb.append(l.mkComment(sr.getNom() + '^'
									+ a.getNom() + " affecte par ailleurs"));
						} else {
